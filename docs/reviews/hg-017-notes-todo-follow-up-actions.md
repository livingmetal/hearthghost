...
- [ ] Android real-device: `메모해:` creates a NOTE in the expected scope and returns a short reference without invoking the LLM.
- [ ] Android real-device: `메모 목록` lists at most 10 NOTE records with short references.
- [ ] Android real-device: `메모 삭제: <short-ref>` deletes only a NOTE in the caller's resolved scope and cannot delete semantic memory.
...
- [ ] Verify a note/todo reference from another scope cannot be read, completed, or deleted.
...
- [x] Add explicit note listing/deletion commands with scope and NOTE-kind checks.
...
- [>] Versioned PostgreSQL migrations are implemented on stacked HG-018 / PR #7 and must be validated before this item is closed.
