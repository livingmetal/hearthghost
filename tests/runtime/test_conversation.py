...
class ConversationTests(unittest.TestCase):
    ...
    def test_node_gateway_admission_and_exact_capability_are_required(self):
        for node, reason in (
            (
                ...
                ConversationReason.NOT_ADMITTED,
            ),
            (
                AdmittedConversationNode(
                    True,
                ...
                ConversationReason.CAPABILITY_MISMATCH,
            ),
        ):
    ...
    def test_text_turn_transitions_thinking_speaking_and_engaged(self):
        opened = self.conversation.open(self.node)
        accepted = self.conversation.accept_text(
            self.node,
            opened.session.session_id,
            "  How was my schedule today?  ",
        )
        completed = self.conversation.complete_response(
            self.node,
            opened.session.session_id,
    ...
    def test_malformed_text_fails_without_changing_session_state(self):
        ...
        for text in ("", "   ", "bad\x00text", "x" * 4_001):
            with self.subTest(text_length=len(text)):
                result = self.conversation.accept_text(
                    self.node,
                    opened.session.session_id,
                    text,
                )
        ...
        stored = self.repository.get(opened.session.session_id)
        self.assertEqual(stored.state, ConversationState.LISTENING)

    def test_other_technical_session_cannot_continue_conversation(self):
        opened = self.conversation.open(self.node)
        other_session = AdmittedConversationNode(
            True,
            self.node.node_id,
            "node-session-2",
            "conversation.text",
    ...
    def test_response_cannot_complete_before_input(self):
        ...
        result = self.conversation.complete_response(
            self.node,
            opened.session.session_id,
            "unexpected",
        )
